...
public class Bag<T> {
    ...
    public int size() { return elements.values().stream().mapToInt(Integer::intValue).sum(); }

    // This method should merge the elements of otherBag into the current bag.
    public void merge(Bag<T> otherBag) { otherBag.elements.forEach((item, count) -> elements.merge(item, count, Integer::sum)); }
    ...
    public Bag<T> distinct() {
        ...
        elements.keySet().forEach(result::add);
        return result;
    }
